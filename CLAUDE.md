...
`NewUnlockWindow` (in `unlock.go`) checks whether a locked identity exists in Vault asynchronously, then swaps the window body to show either a "set password" form (first run, with confirmation) or a "enter password" form (returning user). On submit, calls `core.InitIdentity` in a goroutine, then `onUnlock(app)` on the main thread. Both forms carry an "Import recovery key…" button that swaps the body to an import form (file picker + export passphrase + new unlock password) driving `core.ImportIdentity`; its mismatch error triggers a confirm-and-force retry. "Back" restores the captured password form.
